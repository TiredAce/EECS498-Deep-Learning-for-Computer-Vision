...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_clone, x_test_clone = x_train.clone(), x_test.clone()
    x_train_flat = x_train_clone.view(num_train, -1)
    x_test_flat = x_test_clone.view(num_test, -1)
    for i in range(num_train):
        for j in range(num_test):
            dists[i][j] = torch.sqrt(torch.sum((x_train_flat[i] - x_test_flat[j]) ** 2))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_flat = x_train.clone().reshape(num_train, -1)
    x_test_flat = x_test.clone().reshape(num_test, -1)
    for i in range(num_train):
        dists[i] = torch.sum((x_test_flat - x_train_flat[i]) ** 2, dim = 1) ** 0.5
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    A = x_train.clone().view(num_train, -1)
    B = x_test.clone().view(num_test, -1)
    AB2 = ( A.mm(B.T) ) * 2
    dists = torch.sqrt((A ** 2).sum(dim = 1).reshape(-1, 1) - AB2 + (B ** 2).sum(dim = 1).reshape(1, -1))
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    for j in range(num_test):
        # Find the indices of the k nearest neighbors for the j-th test sample
        ...
        label_counts = torch.bincount(k_nearest_labels, minlength=y_train.max())
        
        max_count_label = torch.argmax(label_counts)
       
        # Assign the predicted label to the j-th test sample
        y_pred[j] = max_count_label
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for num_fold in range(num_folds):
            ...
            x_train_fold = torch.cat([x_train_folds[i] for i in range(num_folds) if i != num_fold], dim=0)
            y_train_fold = torch.cat([y_train_folds[i] for i in range(num_folds) if i != num_fold], dim=0)
            x_val_fold = x_train_folds[num_fold]
            y_val_fold = y_train_folds[num_fold]
            
            # Train KnnClassifier
            classifier = KnnClassifier(x_train_fold, y_train_fold)
            
            # Compute accuracy on validation set
            accuracy = classifier.check_accuracy(x_val_fold, y_val_fold, k)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    # Replace "pass" statement with your code
    best_acc = 0
    for key, val in k_to_accuracies.items():
        maxv = max(val)
        if maxv > best_acc:
            best_acc, best_k = maxv, key
